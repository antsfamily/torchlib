...
class EntropyLoss(th.nn.Module):
    r"""compute the entropy loss of the inputs
    ...
    ::
        ...
        th.manual_seed(2020)
        X = th.randn(1, 3, 4, 2)
        ent_func = EntropyLoss(mode='shannon', axis=(1, 2), caxis=-1, reduction='mean')
        V = ent_func(X)
        print(V)

        X = X[:, :, :, 0] + 1j * X[:, :, :, 1]
        ent_func = EntropyLoss(mode='shannon', axis=(1, 2), caxis=None, reduction='mean')
        V = ent_func(X)
        print(V)

        ent_func = EntropyLoss(mode='shannon', axis=None, caxis=None, reduction='mean')
        V = ent_func(X)
        print(V)

        ent_func = EntropyLoss(mode='shannon', axis=(2), caxis=None, reduction='mean')
        V = ent_func(X)
        print(V)

        ent_func = EntropyLoss(mode='shannon', axis=(2), caxis=None, reduction=None)
        V = ent_func(X)
        print(V)
