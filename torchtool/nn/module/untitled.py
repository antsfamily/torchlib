...
class BalaConv2d(_ConvNd):
    ...
    * :attr:`groups` controls the connections between inputs and outputs.
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        self.gen = np.random.randn()
        self.prob = 0.2

    @weak_script_method
    def forward(self, input):
        input_ones = torch.ones_like(input)
        # g = self.gen.sample().abs()
        # self.alpha = torch.bernoulli(g)
        self.alpha = torch.from_numpy(np.random.binomial(1, self.prob, self.weight.size()))
        self.alpha = torch.as_tensor(self.alpha, dtype=self.weight.dtype, device=self.weight.device)
